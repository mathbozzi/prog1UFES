...
-- funcao que recebe uma lista de Floats e uma media e calcula o desvio padrao
standartDeviation :: [Float] -> Float -> Float
standartDeviation xs media = sqrt.average.map ((^2).(-) media) $ xs
